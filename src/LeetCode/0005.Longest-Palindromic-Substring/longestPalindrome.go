...
func longestPalindrome(s string) string {
	...
	for i := 0; i < len(s); i++ {
		...
		//记录最长回文字串的长度和相应它在原始字符串中的起点
		if dp[i] > maxLen {
			maxLen = dp[i]
			begin = (i - maxLen) / 2 //这里除以2 因为我们插入的辅助字符#
		}
	}
...
// 解法二 滑动窗口，时间复杂度O(n^2),空间复杂度O(1)
func longestPalindrome1(s string) string {
	if len(s) == 0 {
		return ""
	}
	left, right, p1, pr := 0, -1, 0, 0
	for left < len(s) {
		//移动到相同字母的最右边(如果有相同字母)
